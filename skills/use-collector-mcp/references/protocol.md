...
`continue_other_sources` for an unavailable independent source,
`configure_gateway_official_provider` for a missing Gateway-only Official Provider credential,
`stop_platform_action` for login / verification / rate-limit safety stops, and
`reconcile_submission` for an uncertain POST.
...
| `official_provider_credential_required` | Core reports that a Gateway-only Official Provider is not configured | Configure the provider at the local Gateway boundary; never request or paste the platform Secret into the Agent, and never fall back to a browser |

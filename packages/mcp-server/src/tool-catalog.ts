...
const TOOL_MAPPINGS = Object.freeze([
  ...
  tool('collector_xiaohongshu_public_notes_search', 'xiaohongshu.search.public_notes.v1',
    ...
    'Queue one trusted in-page public-note search using an already-open public Xiaohongshu context. Set maximumDetails > 0 for note details; details alone never collect comments. To collect comments in this same operation, explicitly pass comments.maximumScrolls (and optionally comments.replies.maximumThreads). Core enforces the bounded detail and discussion budgets.'),
  ...
  tool('collector_xiaohongshu_note_public_detail', 'xiaohongshu.note.public_detail.v1',
    ...
    'Queue detail-only collection for one visible ranked note in an existing public search or profile page. This Tool does not collect comments. If comments are requested after staged detail, call the comments Tool immediately while the same-document note overlay remains open.'),
  tool('collector_xiaohongshu_note_public_comments', 'xiaohongshu.note.public_comments.v1',
    ...
    'Queue bounded public comment collection from exactly one already-open same-document public note overlay. Zero eligible overlays returns existing_public_note_overlay_required; multiple eligible overlays returns existing_public_note_overlay_ambiguous. Prefer the search Tool comments field when the comment budget is known before search.'),
  tool('collector_xiaohongshu_note_public_comment_replies', 'xiaohongshu.note.public_comment_replies.v1',
    ...
    'Queue bounded public reply-thread collection from exactly one already-open same-document public note overlay with public reply context. Zero or multiple eligible overlays return the corresponding existing_public_note_overlay_* terminal.'),
